...
var start_experiment_procedure = {
  ...
  choices: [" "],
  prompt: "<p>左と右に長方形が出てきます。左の長方形には矢羽がついています。<br/><br/>"+
    "右（矢羽がない方）の長方形の垂直方向の長さが左（矢羽がある方）の長方形の垂直方向の長さよりも短く見えれば「左矢印キー」を、右の長方形の垂直方向の長さが左の長方形の垂直方向の長さよりも長く見えれば「右矢印キー」を押してください。<br/><br/>"+
    "または、左右どちらの長方形の垂直方向の長さの方が長く見えるか、左右の矢印キーで回答してください（意味は同じです）。<br/><br/>"+
    "左右の長方形の垂直方向の長さが同じ長さに見える場合でも、どちらが長く見えるか無理矢理判断して回答してください。<br/><br/>"+
    "判断するのは右図の赤い矢印の方向の長さです。<img src = "../WM教示縦.png"><br/><br/>"+
    "スペースキーを押すと始まります。"+
    "</p>"
};
...
var finish_experiment_procedure = {
  ...
  on_start: function(trial) {
    ...
    trial.stimulus = '<p>実験終了です。</p><p>W・M-constant.csvというファイル名のデータファイルが自動的にダウンロードされています。Excelで開けるので確認してください。</p>'+
